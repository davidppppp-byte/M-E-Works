// ============================================================
// app.js — 蒙恩水電保險管理平台 v2
// 架構：以名稱前綴【水】【電】自動分組，廠區從名稱推導
// ============================================================
...
const PALETTE = ['#1D9E75','#378ADD','#BA7517','#D4537E','#7F77DD','#D85A30','#3B6D11','#A32D2D','#1A7A8A','#8A5C1A'];
let elecChart = null, waterChart = null;

// ── 廠區分組邏輯 ─────────────────────────────────────────────
const LOCATION_KEYS = ['北屯總部','崇德門市','生產中心','物流中心','大雅宿舍','前村路','宿舍'];

function getLocation(siteName) {
  for (const k of LOCATION_KEYS) {
    if (siteName.includes(k)) return k;
  }
  return siteName.replace(/【水】|【電】/g,'').trim();
}

function getType(siteName) {
  if (siteName.startsWith('【水】')) return 'water';
  if (siteName.startsWith('【電】')) return 'elec';
  return 'unknown';
}
...
function syncSelects() {
  const inputType = inputMode === 'water' ? 'water' : 'elec';
  const filteredSites = S.sites.filter(s => getType(s.name) === inputType);
  const inSite = document.getElementById('in-site');
  if (inSite) {
    const v = inSite.value;
    inSite.innerHTML = filteredSites.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
    if (v) inSite.value = v;
  }

  const histF = document.getElementById('hist-filter');
  if (histF) {
    const v = histF.value;
    histF.innerHTML = '<option value="all">所有計費項目</option>' +
      S.sites.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
    if (v) histF.value = v;
  }

  const locations = [...new Set(S.sites.map(s => getLocation(s.name)))].sort();
  const sf = document.getElementById('site-filter');
  if (sf) {
    const v = sf.value;
    sf.innerHTML = '<option value="all">所有廠區</option>' +
...
const mkChartOpts = (yLabel) => ({
  ...
  plugins: {
    legend: { position: 'bottom', labels: { font: { size: 11 }, boxWidth: 10, padding: 10, usePointStyle: true } },
    tooltip: { callbacks: { label: ctx => ` ${ctx.dataset.label}：${ctx.parsed.y?.toLocaleString()} ${yLabel}` } }
  },
  scales: {
...
function getBiVal(siteId, biPeriod, key) {
  const [y, m] = biPeriod.split('-').map(Number);
  const p1 = `${y}-${String(m-1).padStart(2,'0')}`;
  const p2 = biPeriod;
  const r1 = S.records.find(r => r.siteId === siteId && r.period === p1);
  const r2 = S.records.find(r => r.siteId === siteId && r.period === p2);
  const v = (r1?.[key] || 0) + (r2?.[key] || 0);
  return v || null;
}

function getBiPeriods() {
  const allPeriods = [...new Set(S.records.map(r => r.period))].sort();
  const biPeriods = [];
  for (let i = allPeriods.length - 1; i >= 0 && biPeriods.length < 12; i--) {
    const [y, m] = allPeriods[i].split('-').map(Number);
    const biKey = m % 2 === 0 ? allPeriods[i] : `${y}-${String(m+1).padStart(2,'0')}`;
    if (!biPeriods.includes(biKey)) biPeriods.unshift(biKey);
  }
  return biPeriods;
}
...
function renderDash() {
  ...
  if (totalAlerts > 0) {
    badge.style.display = 'inline-flex'; badge.className = 'pill danger';
  ...
  S.records.filter(r => r.period.startsWith(thisYear)).forEach(r => {
    if (allSites.find(s => s.id === r.siteId)) {
      totalElec  += r.elec  || 0;
      totalWater += r.water || 0;
    }
  });

  document.getElementById('metrics').innerHTML = `
    <div class="metric-card"><div class="metric-icon mi-teal"><i class="ti ti-building"></i></div><div class="metric-label">計費項目</div><div class="metric-value">${allSites.length}</div><div class="metric-delta">個</div></div>
    <div class="metric-card"><div class="metric-icon mi-blue"><i class="ti ti-bolt"></i></div><div class="metric-label">今年累計用電</div><div class="metric-value">${totalElec.toLocaleString()}</div><div class="metric-delta">度</div></div>
    <div class="metric-card"><div class="metric-icon mi-teal"><i class="ti ti-droplet"></i></div><div class="metric-label">今年累計水費</div><div class="metric-value">$${totalWater.toLocaleString()}</div><div class="metric-delta">元</div></div>
  ...
  document.getElementById('location-cards').innerHTML = locations.map(loc => {
    ...
    const chips = [
      ...waterSites.map(s => {
        ...
        return `<div class="meter-chip water"><span class="meter-label">${s.name.replace('【水】','').trim()}</span><span class="meter-val">${last ? '$'+last.water.toLocaleString() : '—'}</span></div>`;
      }),
      ...elecSites.map(s => {
        const recs = siteRecs(s.id); const last = recs[recs.length-1];
        return `<div class="meter-chip elec"><span class="meter-label">${s.name.replace('【電】','').trim()}</span><span class="meter-val">${last ? last.elec.toLocaleString()+'度' : '—'}</span></div>`;
      }),
    ].join('');
    return `<div class="loc-card"><div class="loc-card-header"><span class="loc-name"><i class="ti ti-building"></i>${loc}</span><span class="loc-count">${locSites.length} 個項目</span></div><div class="meter-chips">${chips}</div></div>`;
  }).join('');

  // 趨勢圖
  const biPeriods = getBiPeriods();
  const elecSites  = allSites.filter(s => getType(s.name) === 'elec').slice(0,8);
  const waterSites = allSites.filter(s => getType(s.name) === 'water').slice(0,8);
  const mkDs = (siteList, key) => siteList.map((s,i) => ({
    label: s.name.replace(/【水】|【電】/,'').trim(),
    data: biPeriods.map(p => getBiVal(s.id, p, key)),
  ...
  elecChart  = new Chart(document.getElementById('c-elec'),  { type:'line', data:{ labels:biPeriods, datasets:mkDs(elecSites,'elec') },  options:mkChartOpts('度') });
  waterChart = new Chart(document.getElementById('c-water'), { type:'line', data:{ labels:biPeriods, datasets:mkDs(waterSites,'water') }, options:mkChartOpts('元') });
}
...
function renderHistory() {
  ...
  let recs = [...S.records].filter(r => {
    ...
    if (search) {
      ...
      if (!s?.name.toLowerCase().includes(search) && !r.period.includes(search)) return false;
    }
    return true;
  }).sort((a,b) => b.period.localeCompare(a.period));

  const total = recs.length;
  const paged = recs.slice(0, histPage * HIST_PAGE_SIZE);
  document.getElementById('hist-tbody').innerHTML = paged.length
    ? paged.map(r => {
        const s = getSite(r.siteId);
        const type = s ? getType(s.name) : 'unknown';
        const val  = type === 'water'
          ? `<span class="type-badge water">水</span> $${r.water.toLocaleString()}`
          : `<span class="type-badge elec">電</span> ${r.elec.toLocaleString()}度`;
        return `<tr><td style="font-weight:500">${s?s.name:'—'}</td><td>${r.period}</td><td>${val}</td><td><button class="icon-btn del" data-delrec="${r.id}"><i class="ti ti-trash"></i></button></td></tr>`;
      }).join('')
  ...
  if (paged.length < total) {
    ...
    moreBtn.textContent = `查看更多紀錄（還有 ${total - paged.length} 筆）`;
...
function renderIns() {
  ...
  document.getElementById('ins-tbody').innerHTML = sorted.map(i => {
    ...
    return `<tr><td style="font-weight:500">${i.name}</td><td style="color:#666">${i.vendor}</td><td>${i.expiry}</td><td><span class="pill ${pc}">${lb}</span></td><td style="color:#666;font-size:12px">${i.note||'—'}</td><td><button class="icon-btn del" data-delins="${i.id}"><i class="ti ti-trash"></i></button></td></tr>`;
  }).join('');
}
...
function renderSites() {
  const grouped   = groupByLocation(S.sites);
  const locations = Object.keys(grouped).sort();
  document.getElementById('sites-tbody').innerHTML = locations.map(loc => {
    const locSites = grouped[loc];
    const rows = locSites.map(s => {
      const c    = S.records.filter(r => r.siteId === s.id).length;
      const type = getType(s.name);
      return `<tr>
        <td style="padding-left:2rem;color:#555">${s.name}</td>
        <td><span class="type-badge ${type}">${type==='water'?'水':'電'}</span></td>
        <td><span class="pill ${s.cycle===1?'monthly':'bimonthly'}">${s.cycle===1?'每月':'雙月'}</span></td>
        <td>${c}</td>
        <td><button class="icon-btn del" data-delsite="${s.id}"><i class="ti ti-trash"></i></button></td>
      </tr>`;
    }).join('');
...
async function addSite() {
  ...
  const loc   = document.getElementById('new-site-loc').value.trim();
  ...
  const prefix = type === 'water' ? '【水】' : '【電】';
  const name   = label ? `${prefix}${loc} ${label}` : `${prefix}${loc}`;
...
async function handleFileImport(input) {
  ...
  try {
    ...
    setSyncStatus('busy'); let count = 0;
    if (type === 'utility') {
      for (const r of rows) {
        const siteName = String(r['計費項目名稱']||'').trim();
        const period   = String(r['期別(YYYY-MM)']||'').trim();
        const site     = S.sites.find(s => s.name === siteName);
        if (!site || !period) continue;
        await upsertRecord(site.id, period, parseFloat(r['用電度數(度)'])||0, parseFloat(r['水費(元)'])||0);
        count++;
      }
    } else if (type === 'insurance') {
      for (const r of rows) {
        const name = String(r['保險名稱']||'').trim(); const expiry = String(r['到期日(YYYY-MM-DD)']||'').trim();
        if (!name || !expiry) continue;
        await insertInsurance({ name, vendor:String(r['保險公司']||'').trim(), expiry, alertDays:parseInt(r['預警天數'])||30, note:String(r['備註']||'').trim() });
        count++;
      }
    } else if (type === 'sites') {
      for (const r of rows) {
        const name = String(r['計費項目名稱（含【水】【電】前綴）']||'').trim();
        if (!name || S.sites.find(s => s.name === name)) continue;
        await insertSite(name, parseInt(r['計費週期(1=每月/2=雙月)'])||2);
        count++;
      }
    }
    input.value = '';
    syncSelects(); updateIOCounts(); setSyncStatus('ok');
    showToast(`成功匯入 ${count} 筆資料`); renderDash();
  } catch (err) { setSyncStatus('err'); showToast('匯入失敗：' + err.message, 'error'); }
}

function exportExcel(type) {
  if (type === 'utility') {
    const rows = [['計費項目名稱','期別','用電度數(度)','水費(元)']];
    S.records.forEach(r => { const s = getSite(r.siteId); if (s) rows.push([s.name, r.period, r.elec, r.water]); });
    dlXLSX(rows, '水電紀錄', `蒙恩_水電紀錄_${todayStr()}.xlsx`, [{wch:26},{wch:14},{wch:14},{wch:12}]);
  } else if (type === 'insurance') {
...
function exportJSON() {
  ...
  a.href = URL.createObjectURL(blob); a.download = `蒙恩_完整備份_${todayStr()}.json`;
  a.click(); URL.revokeObjectURL(a.href); showToast('完整備份匯出完成');
}
...
document.addEventListener('click', e => {
  ...
  const sb = e.target.closest('.seg-btn');       if (sb)  { document.querySelectorAll('.seg-btn').forEach(b=>b.classList.remove('active')); sb.classList.add('active'); renderDash(); return; }
...
// ── 初始化 ────────────────────────────────────────────────────
const _now = new Date();
const _pe  = document.getElementById('in-period');
if (_pe) _pe.value = `${_now.getFullYear()}-${String(_now.getMonth()+1).padStart(2,'0')}`;
setInputMode('water');
loadAll();
